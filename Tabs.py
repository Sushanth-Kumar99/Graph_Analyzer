...
from graph_function import output_nodes_and_edges, count_nodes
from graph_function import find_density, check_path, is_empty, specific_node, is_directed, show_shortest_paths
...
def create_node():
    ...
    type_node = st.selectbox("Specify the type of node", ["Node", "Person", "Sensor", "Resourse"])
        ...
        for node in st.session_state["node_list"]:
            graph_visualisation_nodes.append(
                Node(
                    ...
                    size=25,
                    shape="circularImage",
                    image="https://t4.ftcdn.net/jpg/00/65/77/27/360_F_"
                          "65772719_A1UV5kLi5nCEWI0BNLLiFaBPEkUbv5Fv.jpg")
            )  # includes **kwargs
...
def analyze_graph():
    ...
    select_function = st.selectbox(label="Select function",
                                   options=["Output nodes and edges", "Count Nodes", "Density of Graph", "Check Path",
                                            "Check if Graph is Empty", "Is Graph Directed", "Specific Node",
    ...
    elif select_function == "Show the shortest path":
